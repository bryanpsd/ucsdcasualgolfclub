...
async function processTournament(tournament, courseName, course) {
  try {
    ...
    if (!resultsExcelRef) {
      console.log(
        `The Tournament "${tournament.fields.title?.['en-US']}" does not have a resultsExcel field.`
      )
      return
    }
    ...
    if (!resultsExcelUrl) {
      console.log(
        `The Tournament "${tournament.fields.title?.['en-US']}" has an invalid resultsExcel asset (missing URL).`
      )
      return
    }
    ...
    for (const row of jsonData) {
      ...
      const resultData = {
        ...
        courseHandicap: isNaN(courseHandicap) ? null : courseHandicap,

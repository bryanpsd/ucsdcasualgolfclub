...
async function pushResultsToContentful() {
  try {
    ...
    if (tournamentEntries.items.length === 0) {
      console.error(`No Tournament found with the title: ${tournamentTitleArg}`)
      return
    }
    ...
    if (!resultsJson) {
      console.error(`The Tournament "${tournamentTitleArg}" does not have a resultsJson field.`)
      return
    }

    const resultsData = typeof resultsJson === 'string' ? JSON.parse(resultsJson) : resultsJson
    ...
    const courseMap = courseEntries.items.reduce((map, item) => {
      ...
      if (courseName) {
        ...
        console.warn(`Course entry missing 'name' field or 'en-US' locale: ${item.sys.id}`)
      }
      return map
    }, {})
    ...
    console.log('All results have been pushed to Contentful and linked to Players.')
  } catch (error) {
